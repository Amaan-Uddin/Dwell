...
export class JoinHouse {
    ...
    async execute(dto: JoinHouseDTO): Promise<void> {
        ...
        if (!dto.houseId.trim()) {
            throw new Error("House ID is required to join that house.")
        }
        if (!dto.userId.trim()) {
            throw new Error("User ID is required to join a house.")
        }
        ...
        if (user.isDeleted()) {
            throw new Error("User account is deleted. Reactivate account to use it.", { cause: "USER_DELETED" })
        }
        if (user.isGuest()) {
            throw new Error("Guest users cannot join houses.", { cause: "GUEST_USER" })
        }

        const house = await this.houseRepo.findById(dto.houseId)
        if (!house) {
            throw new Error("House does not exist.", { cause: "HOUSE_NOT_FOUND" })
        }
        if (house.isArchived()) {
            throw new Error("House is archived, and can no longer be joined.", { cause: "HOUSE_ARCHIVED" })
        }
        ...
        await this.uow.execute(async ({ houseRepo, residentRepo }) => {
            ...
            let resident = await residentRepo.findByUserAndHouseIdForUpdate(user.id, house.id)
